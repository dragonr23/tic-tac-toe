...
export default class App extends React.Component {
  ...
  onTilePress = (row,col) => {
    ...
    //if the value is not 0 in the cell chosen, return
    ...
    //change the current currentPlayer
  ...
  checkTie = () => {
    ...
    // 0 represents empty cells to click on, no 0s means no more cells

    for (let i in cells) {
      for (let j in cells[i]) {
        if (cells[i][i] == 0) {return false;}
      }
    }
    return true;
  }
